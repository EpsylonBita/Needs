...
export const POST = withApiErrorHandler(async (req: NextRequest) => {
  ...
  const { error } = await supabaseAdmin.from('listings').update({ status: 'active' }).eq('id', id)
  if (error) throw ErrorFactory.database(error.message)
  const email = (auth as any).user?.email || 'admin'
  await supabaseAdmin.from('moderation_logs').insert({ admin_email: email, listing_id: id, action: 'unsuspend', admin_notes: note })
